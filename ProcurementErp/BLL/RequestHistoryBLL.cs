...
{
    public class RequestHistoryBLL
    {
        private decimal InitiatorId;
        private RunningRequestViewModel model;
        private decimal RequestType;

        public RequestHistoryBLL(decimal _InitiatorId, RunningRequestViewModel _model, decimal _requestType)
        {
        ...
        public static decimal GetVendorResponseCount(decimal TempId)
        {
            try
            {
                using (var context = new Entities())
                {
                    decimal query = context.SOURCING_CONTACTEDVENDOR.Where(m => m.TEMP_NO == TempId && (m.STATUS != null && m.STATUS != "NEW REQUEST")).Count();
                    return query;
                }
            }
            catch (Exception ex)
